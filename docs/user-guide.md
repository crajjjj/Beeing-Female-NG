...
- **Protected status:** by default matured adults are **Protected**, surviving combat like vanilla followers. To make them killable, set `Global_ProtectGrownAdult=false` in a global add-on INI. This is a single **global** toggle (protection is a base-level flag and all grown children share one base pool, so per-actor control isn't meaningful). Adults that fall back to a copy of a parent's own actor base (when no dedicated adult base is configured) are left untouched, so a parent NPC never inherits the flag.
