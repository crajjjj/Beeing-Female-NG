...
The appeal: the engine handles proximity for free — no scan loop or load-event plumbing to maintain. The cost: cloaks carry a well-known overhead reputation in Skyrim. The effect is applied to *every* nearby actor, so dense crowds (cities, markets, big battles) mean more validation work, and the cloak only ever catches actors that physically come within its radius.
...
In other words, neither approach is free — they just put the cost in different places. The cloak pays in continuous proximity churn that scales with crowd density; PO3 events pay in a cell-entry burst that has to be batched away. Both mods require PO3 regardless, so this is a difference in discovery *mechanism*, not in dependencies.
