...
String Function GetTimeString(Float akTime, Bool akShortFormat = True, String akNegativeText = "") Global
	String sign = ""
	If (akTime < 0.0)
		If akNegativeText;/!=""/; ;Tkc (Loverslab): optimization
	...
	If val;/ != 0/; ;Tkc (Loverslab): optimization
		timeString += val
...
bool function AreModsInstalled(string[] ModNames) global
	;debug.trace("BF: Utility > AreModsInstalled executed")
	int c = ModNames.length
	while c>0
		c-=1
		;if ModNames[c]!="" && IsModInstalled(ModNames[c])==false
		if ModNames[c];/!=""/; ;Tkc (Loverslab): optimization
			;if IsModInstalled(ModNames[c]) ;Tkc (Loverslab): optimization: replaced with ModFile(ModName) function wich will result same
			if ModFile(ModNames[c]) ;Tkc (Loverslab): optimization : ModFile(ModName) will be true if name will be not equal ""
			else;if IsModInstalled(ModNames[c])==false
				return false
			endif
		endif
	endWhile
...
bool function IsModInstalled(string ModName) global
	;debug.trace("BF: Utility > IsModInstalled was executed!")
	...
	endWhile/;
	;;;;; ;Tkc (Loverslab): optimization. standart detection; for reduce size(because using same code) used ModFile() function
	if ModFile(ModName); Tkc: ModFile(ModName) will be true if name will be not equal ""
		...
		return true
	else; Tkc: if will return ""
		;debug.trace("BF: Utility > IsModInstalled ModFile(ModName) is false")	
		return false
	endif
	;;;;;
...
string function ModFile(string ModName) global
	...
	if !(Game.IsPluginInstalled(ModName))
		if !(Game.IsPluginInstalled(ModName+".esp"))
			...
			else;if Game.IsPluginInstalled(ModName+".esm") != 255
				;debug.trace("BF: Utility > ModFile > return =" + ModName+".esm")	
				return ModName+".esm"
			endif
		else;if Game.IsPluginInstalled(ModName+".esp") != 255
			;debug.trace("BF: Utility > ModFile > return =" + ModName+".esp")	
			return ModName+".esp"
		endif
	else;if Game.IsPluginInstalled(ModName) != 255
	...
	if objs.length==0
		...
		while i<c
			if objs[i];/!=none/;
				...
				if PrefareDisplayName ;Tkc (Loverslab): optimization: checks swapped
					if objs[i].GetDisplayName();/!=""/; ;Tkc (Loverslab): optimization
						str+=objs[i].GetDisplayName()
				...
				else;if PrefareDisplayName==false
					if objs[i].GetLeveledActorBase();/!=none/;
						str+=objs[i].GetLeveledActorBase().GetName()
					elseif objs[i].GetDisplayName();/!=""/; ;Tkc (Loverslab): optimization
						str+=objs[i].GetDisplayName()
					else
						str+=objs[i].GetName()
					endif
...
string function GetNames(actor[] Actors) global
	...
	while i<c
		...
		if Actors[i] ;Tkc (Loverslab): optimization
			ActorBase ab = Actors[i].GetLeveledActorBase() ;Tkc (Loverslab): optimization. added because using 3 times
			if ab
				if ab.GetName();/!=""/; ;Tkc (Loverslab): optimization
...
Actor[] function ActorArrayUnique(Actor[] a) global
	...
	int i=0
	while i<c - 1
		if a.find(a[i],i+1) == -1 ;Tkc (Loverslab): optimization
		else;if a.find(a[i],i+1) != -1
			a[i]=none
			newLen-=1
		endif
		i+=1
