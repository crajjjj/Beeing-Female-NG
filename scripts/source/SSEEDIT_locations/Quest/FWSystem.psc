...
Faction[] property ForbiddenFactions auto
...
Keyword[] property ForbiddenKeywords auto
Race[] property ForbiddenRaces auto

function ActorAddSpell(actor a,Spell s, bool PlayerOnly = false, bool bIsCast = false, bool ShowMsg = true) ;Bane --> Edited to allow Spells to be cast in None Locations (Mostly Widerness)
	;if s;/!=none/; && a;/!=none/;
...
function ActorAddSpells(actor a,Spell[] sa, bool PlayerOnly=false, bool bIsCast = false)
...
function ActorAddSpellsS(actor a,string sa, bool PlayerOnly=false, bool bIsCast = false)
	if a ;Tkc (Loverslab): optimization
	else;if a==none
		return
	endif
	...
	if cfg.RelevantPlayer
		if PlayerSex;1-woman
			if PlayerRef.HasSpell(BeeingFemaleSpell)
				...
				ActorAddSpell(PlayerRef,BeeingFemaleSpell, false, false, ShowMsg=cfg.Messages<4) ;Tkc (Loverslab): added ShowMsg parameter to not show messages when Innmersion or None Messages mode
			endIf
		else;0-man
			if PlayerRef.HasSpell(BeeingMaleSpell)
			else
				ActorAddSpell(PlayerRef,BeeingMaleSpell, false, false, ShowMsg=cfg.Messages<4) ;Tkc (Loverslab): added ShowMsg parameter to not show messages when Innmersion or None Messages mode
			endIf			
		endIf	
	endIf
endFunction
...
int function IsValidateFemaleActor(actor a, bool bIgnoreRelevance = false)
	...
	endIf
	if(ForbiddenFactions.Length)
		int ForbiddenFactionsLength = ForbiddenFactions.Length
		int i = 0
		while(i < ForbiddenFactionsLength)
			if a.IsInFaction(ForbiddenFactions[i]) ;|| a.HasKeywordString("SexLabForbid")
				return -1
			endIf
			i += 1
		endWhile
	...
	if(ForbiddenKeywords.Length)
		int ForbiddenKeywordsLength = ForbiddenKeywords.Length
		int i = 0
		while(i < ForbiddenKeywordsLength)
			if a.HasKeyword(ForbiddenKeywords[i]) ;|| a.HasKeywordString("SexLabForbid")
				return -1
			endIf
			i += 1
		endWhile
...
;	endIf
	if ActorRace.IsRaceFlagSet(0x00000004) ;Tkc (Loverslab): optimization: it is looking like messed up and bigger but in result compilled script became smaller and the code will work faster. but it is still quite heavy
		...
		if(ForbiddenRaces.Length)
			int ForbiddenRacesLength = ForbiddenRaces.Length
			int i = 0
			while(i < ForbiddenRacesLength)
				if a.GetRace() == ForbiddenRaces[i]
					return -8
				endIf
				i += 1
			endWhile
