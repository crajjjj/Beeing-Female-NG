...
FormList Property myBFA_ParentRaceList Auto
FormList Property myBFA_BabyMaleList Auto
FormList Property myBFA_BabyFemaleList Auto
FormList Property myBFA_PlayerBabyMaleList Auto
FormList Property myBFA_PlayerBabyFemaleList Auto

ActorBase function GetBabyActor(race ParentRace,int Gender)
	ActorBase m;=none ;Tkc (Loverslab): optimization

	int ParentRaceIndex = myBFA_ParentRaceList.Find(ParentRace)
	
	If ParentRaceIndex == -1
		Debug.Messagebox("Parent race " + ParentRace + " cannot be found in the FormList myBFA_ParentRaceList...")
	Else
		If(Gender == 1)
			m = myBFA_BabyFemaleList.GetAt(ParentRaceIndex) as ActorBase
			
			If m;/!=none/;
				return m
			Else
				Debug.Messagebox("Female baby of the parent race " + ParentRace + " cannot be found in the FormList myBFA_BabyFemaleList...")
			Endif
		Else
			m = myBFA_BabyMaleList.GetAt(ParentRaceIndex) as ActorBase

			If m;/!=none/;
				return m
			Else
				Debug.Messagebox("Male baby of the parent race " + ParentRace + " cannot be found in the FormList myBFA_BabyMaleList...")
			Endif			
		EndIf
	EndIf
...
ActorBase function GetPlayerBabyActor(race ParentRace,int Gender)
	...
	Else
		If(Gender == 1)
			m = myBFA_PlayerBabyFemaleList.GetAt(ParentRaceIndex) as ActorBase
			
			If m;/!=none/;
				return m
			Else
				Debug.Messagebox("Female baby of the parent race " + ParentRace + " cannot be found in the FormList myBFA_PlayerBabyFemaleList...")
			Endif
		Else
			m = myBFA_PlayerBabyMaleList.GetAt(ParentRaceIndex) as ActorBase

			If m;/!=none/;
				return m
			Else
				Debug.Messagebox("Male baby of the parent race " + ParentRace + " cannot be found in the FormList myBFA_PlayerBabyMaleList...")
			Endif			
		EndIf
	EndIf
...
function castCME(actor a, int EffectID, int NumEffects)
	if EffectID==0
		System.ActorAddSpellsS(a,"Always_FollicularPhase");
	elseif EffectID==1
		System.ActorAddSpellsS(a,"Always_Ovulation");
	elseif EffectID==2
		System.ActorAddSpellsS(a,"Always_LuthealPhase");
	elseif EffectID==3
		System.ActorAddSpellsS(a,"Always_PMS");
	elseif EffectID==4
		System.ActorAddSpellsS(a,"Always_Menstruation");
	elseif EffectID==5
		System.ActorAddSpellsS(a,"Always_Trimester1");
	elseif EffectID==6
		System.ActorAddSpellsS(a,"Always_Trimester2");
	elseif EffectID==7
		System.ActorAddSpellsS(a,"Always_Trimester3");
	elseif EffectID==8
		System.ActorAddSpellsS(a,"Always_LaborPains");
	elseif EffectID==9
		System.ActorAddSpellsS(a,"Always_Recovery");
	endIf
		...
		if cmeCount>0
			...
			; there are more effects available then needed. pick random one
				while maxEffects>0
					...
					if spl;/!=none/;
						System.ActorAddSpell(akActor,spl, False, False, False) ;Tkc (Loverslab): added ShowMsg parameter to not show messages when Innmersion or None Messages mode
					endif
				endWhile
			else
...
float function getActorDurationScale(int Step, Actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				result *= getRaceDurationScale(Step, ab.GetRace())
			endIf
		endIf
	endIf
	if result < 0.2
...
float function getActorDuration_BetweenLaborPains(actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				result *= getRaceDuration_BetweenLaborPains(ab.GetRace())
			endIf
		endIf
	endIf
	if result < 0.3
...
float function getActorDuration_BabySpawn(actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				result *= getRaceDuration_BabySpawn(ab.GetRace())
			endIf
		endIf
	endIf
	if result < 0.3
...
float function getActorContraceptionDuration(actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				result *= getRaceContraceptionDuration(ab.GetRace())
			endIf
		endIf
	endIf
	if result < 1.0
...
float function getActorDamageScale(int DamageType, actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				result *= getRaceDamageScale(DamageType, ab.GetRace())
			endIf
		endIf
	endIf
	if result < 0.05
...
float function PMSChanceActorScale(actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				result *= PMSChanceRaceScale(ab.GetRace())
			endIf
		endIf
	endIf
	if result < 0.1
...
bool function ActorCanBecomePregnant(actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				return RaceCanBecomePregnant(ab.GetRace())
			endIf
		endIf
	endIf
	return 1.0
...
float function ActorMaxBabse(actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				result *= RaceMaxBabse(ab.GetRace())
			endIf
		endIf
	endIf
	if result < 0.1
...
float function ActorMaxChance(actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				result *= RaceMaxChance(ab.GetRace())
			endIf
		endIf
	endIf
	if result < 0.1
...
float function ActorSizeScaler(int Type, actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				result *= RaceSizeScaler(Type, ab.GetRace())
			endIf
		endIf
	endIf
	if result < 0.1
...
float function ActorMaleSpermDurationScale(actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				result *= RaceMaleSpermDurationScale(ab.GetRace())
			endIf
		endIf
	endIf
	if result < 0.1
...
float function ActorSpermAmountScale(actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				result *= RaceSpermAmountScale(ab.GetRace())
			endIf
		endIf
	endIf
	if result < 0.1
...
float function ActorBabyHealingScale(actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				result *= RaceBabyHealingScale(ab.GetRace())
			endIf
		endIf
	endIf
	if result < 0.1
...
float function ActorBabyDamageScale(actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				result *= RaceBabyDamageScale(ab.GetRace())
			endIf
		endIf
	endIf
	if result < 0.1
...
float function ActorMaleRecoveryScale(actor a)
	...
	if a ;Tkc (Loverslab): optimization
		...
		if ab
			if ab.GetRace()
				result *= RaceMaleRecoveryScale(ab.GetRace())
			endIf
		endIf
	endIf
	if result < 0.1
