	...
	while i>0
		...
		if PlayerChildren[i];/!=none/;
			if PlayerChildren[i].PlayerLastSeen >0
				if (PlayerChildren[i].GetActorValue(OrderAV()) < 10 && GameDaysPassed.GetValue() - PlayerChildren[i].PlayerLastSeen > 1) || (PlayerChildren[i].GetActorValue(OrderAV()) == 12 && GameDaysPassed.GetValue() - PlayerChildren[i].PlayerLastSeen > 2)
