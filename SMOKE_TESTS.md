...
| 14.1b | P1 | Magic-effect hook is filtered, not global | With BiS installed, bathing still reduces sperm (14.1 passes) — the hook now arrives via PO3 `OnMagicEffectApplyEx` registered for the bathing effects only. Without BiS, no filter is registered and a profiler shows **zero** `FWAbilityBFOnMagicEffectApply` stacks in a crowded hold (pre-3.5.16 showed 10⁵+/hour, one per magic effect per tracked female). MCM "Disable OnMagicEffectApply Events" unregisters the filters outright | FWAbilityBFOnMagicEffectApply, FWAddOnManager `GetMagicEffectFilters`, FWAddOn_Misc `OnRegisterMagicEffectFilters` |
